...
class solopine_facebook_widget extends WP_Widget {
	...
	function solopine_facebook_widget() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'solopine_facebook_widget', __('Florence: Facebook Like Box', 'solopine_facebook_widget'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		?>
			...
			<iframe src="http://www.facebook.com/plugins/likebox.php?href=<?php echo $page_url; ?>&amp;width=<?php echo $width; ?>&amp;colorscheme=light&amp;show_faces=<?php if($faces) { echo 'true'; } else { echo 'false'; } ?>&amp;border_color&amp;stream=<?php if($stream) { echo 'true'; } else { echo 'false'; } ?>&amp;header=<?php if($header) { echo 'true'; } else { echo 'false'; } ?>&amp;height=<?php echo $height; ?>&amp;show_border=false" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:<?php echo $width; ?>px; height:<?php echo $height; ?>px; background:#fff;" allowTransparency="true"></iframe>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Find us on Facebook', 'width' => 300, 'height' => 290, 'header' => 'on', 'faces' => 'on', 'page_url' => '', 'stream' => false);
